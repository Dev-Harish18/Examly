...
exports.submitExam = catchAsync(async (req, res, next) => {
  ...
  for (let i = 0; i < answers.length; i++) {
    ...
    //Keywords check --> 30
    let count = 0;
    const keywords = keys[i].keywords;
    ...
    totalMarks += questions[i].maxMark * 1;
    let finalScore = Math.round((score / 100) * questions[i].maxMark);
    studentMarks += finalScore * 1;

    marks.push((score / 100) * questions[i].maxMark);
  ...
  const status = studentMarks >= exam.minMark ? "pass" : "fail";
